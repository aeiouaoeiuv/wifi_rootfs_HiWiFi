...
function index()
    ...
    entry({"admin_web", "system", "guide1"}, template("admin_web/system/guide1"), _("引导模式1"), 191, guide_access_tag)
    entry({"admin_web", "system", "guide2"}, template("admin_web/system/guide2"), _("引导模式2"), 192, guide_access_tag)
    entry({"admin_web", "system", "guide3"}, template("admin_web/system/guide3"), _("引导模式3"), 193, guide_access_tag)
    entry({"admin_web", "system", "guide_online"}, template("admin_web/system/guide_online"), _("引导模式_上网"), 193, true)
    ...
    entry({"admin_web", "system","upgrade"}, template("admin_web/system/upgrade"), _("路由器固件升级"), 198,true)
    entry({"admin_web", "system","upgrade_user"}, template("admin_web/system/upgrade_user"), _("路由器固件升级"), 198,true)    --不需要登录
    ...
    entry({"admin_web", "system","disk"}, template("admin_web/system/disk"), _("存储诊断"), 192,true).leaf = true
