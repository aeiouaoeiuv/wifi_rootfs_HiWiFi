...
local tonumber, tostring, math
= tonumber, tostring, math
...
function protocol.status(self)
local iface = uci.cursor_state():get_all("network", self.sid)
local proto  = iface["proto"]
local ifname = iface["ifname"]
local device = iface["device"]
local up     = tonumber(iface["up"])
if proto == "pppoe" then
if device == nil  then
return "down"
end
...
return i18n.translate(p.." "..self.sid)
elseif p == "dhcp" then
return i18n.translate(p.." "..self.sid)
else
...
function wifinet.txpwr(self)
return self.iwinfo.txpwr or
tostring(_uci_state:get("wireless", self.iwdata.device, "txpwr"))
end
