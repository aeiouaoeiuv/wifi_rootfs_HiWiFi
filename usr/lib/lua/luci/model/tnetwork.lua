...
local d = defaults()
local z = uci_r:section("tnetwork", "zone", nil, {
name    = n,
network = " ",
...
if uci_r:get("tnetwork", n) == "zone" then
return zone(n)
else
local z
uci_r:foreach("tnetwork", "zone",
function(s)
...
local r = false
if uci_r:get("tnetwork", n) == "zone" then
local z = uci_r:get("tnetwork", n, "name")
r = uci_r:delete("tnetwork", n)
n = z
else
uci_r:foreach("tnetwork", "zone",
function(s)
if n and s.name == n then
r = uci_r:delete("tnetwork", s['.name'])
return false
end
end)
end
if r then
uci_r:foreach("tnetwork", "rule",
function(s)
if s.src == n or s.dest == n then
uci_r:delete("tnetwork", s['.name'])
end
end)
uci_r:foreach("tnetwork", "redirect",
function(s)
if s.src == n or s.dest == n then
uci_r:delete("tnetwork", s['.name'])
end
end)
uci_r:foreach("tnetwork", "forwarding",
function(s)
if s.src == n or s.dest == n then
uci_r:delete("tnetwork", s['.name'])
end
...
uci_r:set("tnetwork", s['.name'], "network", old)
end
uci_r:set("tnetwork", s['.name'], "name", new)
r = true
return false
end
end)
...
function(s)
if s.src == old then
uci_r:set("tnetwork", s['.name'], "src", new)
end
if s.dest == old then
uci_r:set("tnetwork", s['.name'], "dest", new)
end
end)
uci_r:foreach("tnetwork", "redirect",
function(s)
...
if not nfs.access("/etc/config/tnetwork") then
nfs.writefile("/etc/config/tnetwork", "")
end
uci_r:foreach("tnetwork", "defaults",
function(s)
self.sid  = s['.name']
return false
end)
self.sid = self.sid or uci_r:section("tnetwork", "defaults", nil, { })
end
function defaults.get(self, opt)
return _get("tnetwork", self.sid, opt)
end
function defaults.set(self, opt, val)
return _set("tnetwork", self.sid, opt, val)
end
...
if uci_r:get("tnetwork", z) == "zone" then
self.sid  = z
self.data = uci_r:get_all("tnetwork", z)
else
uci_r:foreach("tnetwork", "zone",
function(s)
if s.name == z then
...
local s = uci_r:section("tnetwork", "forwarding", nil, {
src     = self:name(),
dest    = dest
})
...
local name = self:name()
uci_r:delete_all("tnetwork", "forwarding",
function(s)
...
local s = uci_r:section("tnetwork", "redirect", nil, options)
return s and redirect(s)
end
function zone.add_rule(self, options)
options = options or { }
options.src = self:name()
local s = uci_r:section("tnetwork", "rule", nil, options)
return s and rule(s)
end
...
return uci_r:get("tnetwork", self.sid, "src")
end
function forwarding.dest(self)
return uci_r:get("tnetwork", self.sid, "dest")
end
