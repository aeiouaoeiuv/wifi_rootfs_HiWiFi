...
function net.brlantable()
    local data = {}
    local k = { "port no", "mac addr", "is local", "ageing timer" }
    local ps = luci.util.execi("br showmacs br-lan")
    if not ps then
        return
    else
        ps()
...
function process.runing(process_name)
return os.execute("ps | grep -v grep | grep %s >/dev/null" % process_name)
end
...
if pwh == nil or pwh and nixio.crypt(pass, pwh) ~= pwh then
return false
else
return true
end
...
function wifi.getiwinfo2(ifname)
local stat, iwinfo = pcall(require, "iwinfo")
local d, n = ifname:match("^(%w+)%.network(%d+)")
...
function isapplecdn()
return os.execute("grep -qs apple /etc/nginx/vh.phobos_apple_com.conf")
end
function build_time()
local build = fs.readfile("/etc/.build")
if build then
return luci.util.trim(build)
else
return "null"
end
end
